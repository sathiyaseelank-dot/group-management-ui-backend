...
func (s *Server) handleUIGroupsSubroutes(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 1 {
		...
		resRows, _ := db.Query(state.Rebind(`SELECT r.id, r.name, r.type, r.address, r.protocol, r.port_from, r.port_to, r.alias, r.description, r.remote_network_id, r.connector_id, r.firewall_status
			FROM access_rules ar
