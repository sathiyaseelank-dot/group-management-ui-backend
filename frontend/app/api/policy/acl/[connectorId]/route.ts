...
export async function GET(
  ...
  const resourceIndex: Record<string, { address: string; protocol: string; port_from: number | null; port_to: number | null }> = {};
  const aclEntries: { identity: string; resource_id: string }[] = [];

  resources.forEach((res) => {
    resourceIndex[res.resource_id] = {
      address: res.address,
      protocol: res.protocol,
      port_from: res.port_from,
      port_to: res.port_to,
    };
    res.allowed_identities.forEach((identity) => {
      aclEntries.push({ identity, resource_id: res.resource_id });
    });
  });
