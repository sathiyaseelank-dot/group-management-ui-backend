...
export default function ResourcePolicyConfigPage() {
  const { policyId } = useParams();
  const id = Array.isArray(policyId) ? policyId[0] : (policyId as string | undefined);
  ...
  useEffect(() => {
    ...
    if (!id) return;
    setPolicy(getResourcePolicy(id));
  }, [id]);
  ...
  if (!policy) {
    return (
      <div className="space-y-4">
        <Link href="/dashboard/policy/resource-policies">
